...
def optimizar(ruta, hoja, rango, tamaño, cantidad):
    ...
    for combinacion in combinations(indices_filas, tamaño): #combinacion de indices de filas
        ...
        if all(abs(sc) <= rango  for sc in (sumas_combinacion)) and not any(idx in df_filtrado.index for idx in combinacion):
...
def seleccionar_archivo_excel():
    Tk().withdraw()  #solo la caja de diálogo
...
def inyectar_df_en_excel(df, ruta_excel):
    ...
    for sheet in libro.worksheets:
        ...
        for row in range(7, sheet.max_row + 1):
            # Crear la fórmula de suma para cada fila desde B hasta L
            sum_formula = '=SUM({}{}:{}{})'.format(chr(start_col + 64), row, chr(end_col + 64), row)
            # Colocar la fórmula en la columna M (columna 13) de la fila actual
            sheet.cell(row=row, column=13, value=sum_formula)
        # Crear la fórmula de suma para la columna M (columna 13)
        sum_formula = '=SUM({}{}:{}{})'.format(chr(2 + 64), 4, chr(12 + 64), 4)
        # Colocar la fórmula en la celda M4
        sheet.cell(row=4, column=13, value=sum_formula)
...
def solicitar_valores():
    ...
    root = tk.Tk()
    # Esconder la ventana principal (solo queremos mostrar el cuadro de diálogo)
    root.withdraw()

    # Solicitar el primer valor al usuario
    rango = simpledialog.askinteger("Input", "Ingrese el rango para cada combinacion",
                                    parent=root,
                                    minvalue=20, maxvalue=5000)  # Puedes ajustar los valores mínimos y máximos según sea necesario

    # Solicitar el segundo valor al usuario
    cantidad = simpledialog.askinteger("Input", "Ingrese la cantidad de filas a obtener:",
                                       parent=root,
                                       minvalue=2, maxvalue=1000)  # Ajusta los valores mínimos y máximos según sea necesario
...
def main():
    ...
    nombre_hoja = 'Sheet1'
    rango, cantidad = solicitar_valores()
    if archivo_excel and rango and cantidad:
        df_tabla = analizar_excel(archivo_excel, 'G:S')
        ruta = exportar_tabla(df_tabla, archivo_excel)
        df_final = optimizar(ruta, nombre_hoja, rango, 2, cantidad)
        inyectar_df_en_excel(df_final, ruta)
    else:
        print('Datos Faltantes o No ingresados correctamente')
